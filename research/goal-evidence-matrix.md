...
| Inspect wallet access and balances | Rechecked 23:12 UTC 2 Sep 2026. First-client KMS still disabled. Hydra DCR reminted unused Connect code `RCGDQVq9` at 23:12:08 UTC. No `session.json`, no `addresses.json`, no `user-provided.json`. Tokens valid until ~23:41 UTC. | **Not achieved** | Need Connect approval, a real Portal App ID with the device-code grant, or a MetaMask address shared on `/connect`. |
